...
## Overall Goal
The user has a .NET 9.0 project focused on high-performance rendering of Lyapunov fractals using multiple computational approaches: CPU, multi-core CPU, and GPU (OpenCL).
...
- **Build Prerequisites**: Requires .NET SDK 9.0 or higher and OpenCL drivers for GPU acceleration. On macOS, requires setting `DYLD_LIBRARY_PATH` for OpenCL to work properly.
...
- **Technology Stack**: .NET 9.0, OpenCL (via Cloo library), ASP.NET Core for web API, .NET MAUI for cross-platform GUI, System.Threading.Channels for job queuing.
