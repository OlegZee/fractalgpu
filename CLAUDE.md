...
- `LyapRendererBase : FractalRenderer<Lyapunov.Settings>` closes the generic over Lyapunov settings and supplies the coloring. Its subclasses are the backends: `LyapRendererCpu`, `LyapRendererSimd : LyapRendererCpu` (vectorizes the inner loop over adjacent A-axis pixels with variable-width `Vector<double>` + `Vector.Log`; 4/2/1-block interleave cascade for ILP, inherits the scalar code for row tails and non-accelerated hardware; output matches the scalar renderer at float precision — keep that property when touching it), `LyapRendererMulticore<T>` (splits the A-range into tiles — 256 by convention — dispatched over the ThreadPool; `T` is any `LyapRendererBase` with a parameterless ctor, so it composes with the SIMD renderer as `LyapRendererMulticore<LyapRendererSimd>`), and `LyapRendererOpenCl(int deviceIndex = 0)` (builds a `ComputeContext` over the single selected device; the kernel source is the embedded resource `Resources/Lyapunov.c`, loaded via `Resources.cs` using the assembly's `RootNamespace`).
...
`Fractal/DeviceRegistry.cs` exposes the unified, index-addressable device list used by both RenderCli and RenderServer: index 0 = single-core CPU, 1 = multi-core CPU, 2 = single-core SIMD, 3 = multi-core SIMD, 4+ = OpenCL devices in enumeration order. `GetByIndex` throws with a neutral message; `DefaultIndex()` prefers the first GPU, falling back to multi-core SIMD. CLI-specific presentation (printing the device table, the "Run 'list-devices'" hint on errors) deliberately lives in `RenderCli/Program.cs`, not in the library.
