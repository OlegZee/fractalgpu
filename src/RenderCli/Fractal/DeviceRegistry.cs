...
namespace FractalGpu.RenderCli.Fractal
{
	...
	internal sealed record DeviceDescriptor(int Index, DeviceKind Kind, string Name, string Details, Func<LyapRendererBase> CreateRenderer);
	...
	internal static class DeviceRegistry
	{
		...
		{
			var devices = Enumerate(out var openClError);
